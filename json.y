...
json_list: json { json_result = $1; }

json: /* empty */ { $$ = NULL; }
	| value json { $1->sibling_next = $2; $$ = $1; }
...
void
json_encode_puts_raw (struct json_encode_buf *bp, char const *str)
{
	char const *p;
	for (p = str; *p; p++)
		json_encode_put (bp, *p);
}
...
json_encode_str (struct json_encode_buf *bp, char const *str)
{
	...
	json_encode_put (bp, '"');
	if (str) {
		for (p = (unsigned char const *)str; *p; p++) {
			c = *p;
			if (c >= ' ' && c < 0x7f
			    && c != '\\'
			    && c != '"') {
				json_encode_put (bp, c);
				   ...
				   && (p[1] & 0xc0) == 0x80) {
				json_encode_put (bp, p[0]);
				json_encode_put (bp, p[1]);
				p++;
